...
class Points : Drawable {
    ...
    func drawPoint(p: SIMD3<Float>, renderEncoder: MTLRenderCommandEncoder, mvp: inout MVPMatrices)
    {
        drawablePoint.color = [1.0, 0.0, 0.0, 0.0]
        drawablePoint.move(to: p)
        drawablePoint.draw(renderEncoder: renderEncoder, mvp: &mvp)
    }
    
    func drawLine(p1: SIMD3<Float>, p2: SIMD3<Float>, renderEncoder: MTLRenderCommandEncoder, mvp: inout MVPMatrices)
    {
    ...
    func drawPoints(points: [SIMD3<Float>], renderEncoder: MTLRenderCommandEncoder, mvp: inout MVPMatrices) -> [SIMD3<Float>]
    {
        assert(points.count > 1)
        var interpolatedPoints: [SIMD3<Float>] = []

        drawPoint(p: points.last!, renderEncoder: renderEncoder, mvp: &mvp)

        for i in 1..<points.count {
            let p1 = points[i-1]
            let p2 = points[i]

            drawPoint(p: p1, renderEncoder: renderEncoder, mvp: &mvp)
            drawLine(p1: p1, p2: p2, renderEncoder: renderEncoder, mvp: &mvp)
    ...
    func draw(renderEncoder: MTLRenderCommandEncoder, mvp: inout MVPMatrices) {
        ...
        repeat {
            points = drawPoints(points: points, renderEncoder: renderEncoder, mvp: &mvp)
